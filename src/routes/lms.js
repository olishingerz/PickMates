...
async function refreshFixtureCache(gameId, week) {
  ...
  await pool.query(
    `INSERT INTO lms_weeks (game_id, week_number, fixtures_cache, deadline)
     VALUES ($1,$2,$3,$4)
     ON CONFLICT (game_id, week_number) DO UPDATE
       SET fixtures_cache = $3,
           deadline = COALESCE(lms_weeks.deadline, $4)`,
    [gameId, week, JSON.stringify(fixtures), suggestedDeadline]
  );
  return fixtures;
}
...
async function getLmsData(gameId, userId) {
  const [gameRes, participantsRes, weeksRes, picksRes] = await Promise.all([
    ...
    `, [gameId]),
    pool.query(`
      SELECT u.id AS user_id, u.username, gp.draft_position, gp.team_name, gp.is_co_host, gp.has_paid
