...
async function getScorecardData(gameId, userId) {
  const [gameRes, teamsRes, participantsRes, holesRes, scoresRes] = await Promise.all([
    pool.query(
  ...
  return { game, teams, standings, holes, unassignedParticipants, userId };
}
